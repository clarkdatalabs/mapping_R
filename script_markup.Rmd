...
api.key.install("<ACS API key>")
```
```{r, include=FALSE} <!-- Dan's API key for generating the markdown -->
...
```
<!-- PLOT FROM LOCAL: ![US county chloropleth using ACS per capita income data](figures/county_choropleth_acs.png) -->
...
In this example we will work with the data provided along with this tutorial. Make sure you have unzipped the folder county_census before proceeding!

The following prompts you to select the provided county census shapefiles at the path ...county_census/County_2010Census_DP1.shp. Note: You will have to unzip the folder county_census first!
